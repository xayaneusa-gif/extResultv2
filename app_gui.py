...
def get_all_semesters(run_folder):
    ...
    for sem in data["data"]:
        ...
        es_id = sem.get("esId")
        sm_id = sem.get("smId")
        exam_name = sem.get("examName")

        label = f"{name} - {status} | Exam:{exam_name} | esId:{es_id} | smId:{sm_id}"
        ...
        item_value = {
            ...
            "examScheduleName": sem.get("examScheduleName"),
            "degreeName": sem.get("degreeName"),
            "instituteName": sem.get("instituteName"),
        }
...
def step4_fetch_range(start_roll, end_roll, run_folder, progress_callback=None):
    try:
        ...
        range_meta_path = os.path.join(run_folder, "range_meta.json")
        requested_range = {"start_roll": int(start_roll), "end_roll": int(end_roll)}

        # If same range is requested and data.xml already exists, reuse it.
        if os.path.exists(xml_path) and os.path.exists(range_meta_path):
            try:
                with open(range_meta_path, "r", encoding="utf-8") as f:
                    prev = json.load(f)
                if int(prev.get("start_roll", -1)) == requested_range["start_roll"] and int(prev.get("end_roll", -1)) == requested_range["end_roll"]:
                    if progress_callback:
                        progress_callback("Range unchanged. Reusing existing data.xml.")
                    return {"status": "success", "xml": xml_path, "cached": True}
            except Exception:
                pass
        ...
        with open(range_meta_path, "w", encoding="utf-8") as f:
            json.dump(requested_range, f, indent=2)
...
def step5_fetch_dmc(run_folder, sem_data, progress_callback=None, reappear_update_mode="update_latest_non_empty"):
    try:
        ...
        fetch_id = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        fetch_root = os.path.join(run_folder, f"fetch_{fetch_id}")
        os.makedirs(fetch_root, exist_ok=True)
        if progress_callback and isinstance(sem_data, dict):
            progress_callback(
                "Fetch start context: "
                f"mode={reappear_update_mode}, semName={sem_data.get('semName')}, "
                f"semNo={sem_data.get('semNo')}, smId={sem_data.get('smId')}, "
                f"esId={sem_data.get('esId')}, examScheduleName={sem_data.get('examScheduleName')}, "
                f"fetchFolder={fetch_root}"
            )
        ...
        if progress_callback:
            for idx, it in enumerate(semester_items, start=1):
                progress_callback(
                    f"Resolved semester item {idx}/{total_semesters}: "
                    f"semName={it.get('semName')}, semNo={it.get('semNo')}, "
                    f"smId={it.get('smId')}, esId={it.get('esId')}, examTypeKey={it.get('examTypeKey')}, "
                    f"examScheduleName={it.get('examScheduleName')}"
                )
        selected_context = {
            "requested_sem_data": sem_data if isinstance(sem_data, dict) else {},
            "resolved_mode": mode,
            "resolved_semester_items": semester_items,
        }
        ...
        def folder_for_sem(sem_item, sem_index):
            ...
            if isinstance(sem_data, dict) and sem_data.get("mode") == "all_semesters":
                return os.path.join(fetch_root, f"sem_{sem_no or sem_index}_{sm_id}_{es_id}")
            return os.path.join(fetch_root, f"sem_{sm_id}_{es_id}")
        ...
        if mode == "single_latest":
            ...
            target_es = sem_data.get("esId") if isinstance(sem_data, dict) else None
            if not target_sm and semester_items:
                target_sm = semester_items[0].get("smId")
            if not target_es and semester_items:
                target_es = semester_items[0].get("esId")
            final_folder = os.path.join(fetch_root, f"sem_{target_sm}_{target_es}")
            os.makedirs(final_folder, exist_ok=True)
            try:
                with open(os.path.join(final_folder, "_selection_context.json"), "w", encoding="utf-8") as f:
                ...
                for sem_index, sem_item in enumerate(semester_items, start=1):
                    ...
                    try:
                        ...
                        if reappear_update_mode == "update_latest_non_empty":
                            # Mode 1: keep upgrading until a clear pass is reached.
                            # This supports 3rd/4th/... reappear attempts robustly.
                            if chosen_class in {"status-reappear", "status-fail", "status-rl", "status-ab", "status-neutral"}:
                                chosen_payload = parsed
                                chosen_class = new_class
                                chosen_attempt_no = sem_index
                        else:
        ...
        for sem_index, sem_item in enumerate(semester_items, start=1):
            ...
            try:
                with open(os.path.join(sem_folder, "_selection_context.json"), "w", encoding="utf-8") as f:
                    json.dump(selected_context, f, indent=2)
            except Exception:
                pass
        ...
        if mode == "all_semesters":
            return {"status": "success", "folder": fetch_root, "mode": "all_semesters"}
        sem_sm = sem_data.get("smId") if isinstance(sem_data, dict) else "na"
        sem_es = sem_data.get("esId") if isinstance(sem_data, dict) else "na"
        return {"status": "success", "folder": os.path.join(fetch_root, f"sem_{sem_sm}_{sem_es}"), "mode": "single"}
...
def get_result_class(result_text):
    ...
    if not value:
        return "status-neutral"
    # Short codes from core JSON
    if value in {"p", "pass"}:
        return "status-pass"
    if value in {"re", "r", "reappear", "re-appear"}:
...
def render_subject_scorecards(subject_entries, subject_rank_lookup, subject_avg_lookup):
    ...
    for entry in subject_entries:
        ...
        # Show fail-component details only when downloaded subject result is Re-appear.
        reappear_fail_html = ""
        if result_class == "status-reappear":
            theory_external = entry.get("theory_external", {}) or {}
            practical_external = entry.get("practical_external", {}) or {}
            practical_internal = entry.get("practical_internal", {}) or {}

            ext_ob = _sum_mo(theory_external)
            ext_pm = _mpm(theory_external)
            ext_fail = ext_pm > 0 and ext_ob < ext_pm

            prac_ob = _sum_mo(practical_external) + _sum_mo(practical_internal)
            prac_pm = _mpm(practical_external) + _mpm(practical_internal)
            prac_fail = prac_pm > 0 and prac_ob < prac_pm

            fail_lines = []
            if ext_fail:
                fail_lines.append(
                    f"<div style=\"border:1px solid #efb0b8;border-radius:10px;padding:8px;background:#ffe5e8;color:#c62828;font-weight:700;\">External FAIL {ext_ob:g}/{ext_pm:g}</div>"
                )
            if prac_fail:
                fail_lines.append(
                    f"<div style=\"border:1px solid #efb0b8;border-radius:10px;padding:8px;background:#ffe5e8;color:#c62828;font-weight:700;\">Practical FAIL {prac_ob:g}/{prac_pm:g}</div>"
                )
...
def build_all_semester_student_reports(run_folder, template, progress_callback=None, tone_mode="motivational", reappear_update_mode="update_latest_non_empty"):
    ...
    for sem_folder in semester_folders:
        ...
        for file_name in sorted(os.listdir(sem_folder)):
...
def step6_generate_html(data_folder, run_folder, progress_callback=None, tone_mode="motivational", report_mode="single", reappear_update_mode="update_latest_non_empty"):
    ...
    if progress_callback:
        progress_callback(
            f"HTML creation context: report_mode={report_mode}, reappear_mode={reappear_update_mode}, data_folder={data_folder}"
        )

    if report_mode == "all_semesters":
        combined_reports = build_all_semester_student_reports(data_folder, template, progress_callback=progress_callback, tone_mode=tone_mode, reappear_update_mode=reappear_update_mode)
...
"""
    ...
    json_files = sorted([file_name for file_name in os.listdir(data_folder) if file_name.endswith(".json")])
    total_files = len(json_files)
    selection_context = {}
    context_path = os.path.join(data_folder, "_selection_context.json")
    if os.path.exists(context_path):
        try:
            with open(context_path, "r", encoding="utf-8") as f:
                selection_context = json.load(f)
        except Exception:
            selection_context = {}
    if progress_callback and selection_context:
        req = selection_context.get("requested_sem_data", {})
        progress_callback(
            "HTML selection context loaded: "
            f"resolved_mode={selection_context.get('resolved_mode')}, "
            f"semName={req.get('semName')}, semNo={req.get('semNo')}, "
            f"smId={req.get('smId')}, esId={req.get('esId')}, "
            f"examScheduleName={req.get('examScheduleName')}"
        )
...
"""
    ...
    req_meta = selection_context.get("requested_sem_data", {}) if isinstance(selection_context, dict) else {}
    resolved_items = selection_context.get("resolved_semester_items", []) if isinstance(selection_context, dict) else []
    resolved_first = resolved_items[0] if isinstance(resolved_items, list) and resolved_items else {}
    fallback_identity = student_snapshots[0] if student_snapshots else {}
    hdr_degree = req_meta.get("degreeName") or resolved_first.get("degreeName") or fallback_identity.get("degree", "-")
    hdr_exam = req_meta.get("examScheduleName") or resolved_first.get("examScheduleName") or fallback_identity.get("exam_schedule", "-")
    hdr_inst = req_meta.get("instituteName") or resolved_first.get("instituteName") or fallback_identity.get("institute", "-")
    hdr_sem = req_meta.get("semName") or resolved_first.get("semName") or fallback_identity.get("semester", "-")
    hdr_attempt_type = req_meta.get("examTypeKey") or resolved_first.get("examTypeKey") or "-"
    hdr_resolved_mode = selection_context.get("resolved_mode", "-") if isinstance(selection_context, dict) else "-"

    universal_exam_info_html = f"""
<div style="margin-bottom:16px;border:1px solid #d8dfeb;border-radius:12px;padding:12px;background:#f8fbff;">
    <div style="font-weight:700;color:#344054;margin-bottom:8px;">Selected Exam Context</div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
        <div><small style="color:#667085;">Degree</small><div style="color:#1d2939;font-weight:600;">{escape(str(hdr_degree))}</div></div>
        <div><small style="color:#667085;">Exam Schedule</small><div style="color:#1d2939;font-weight:600;">{escape(str(hdr_exam))}</div></div>
        <div><small style="color:#667085;">Institute</small><div style="color:#1d2939;font-weight:600;">{escape(str(hdr_inst))}</div></div>
        <div><small style="color:#667085;">Semester</small><div style="color:#1d2939;font-weight:600;">{escape(str(hdr_sem))}</div></div>
        <div><small style="color:#667085;">Exam Type</small><div style="color:#1d2939;font-weight:600;">{escape(str(hdr_attempt_type))}</div></div>
        <div><small style="color:#667085;">Resolve Mode</small><div style="color:#1d2939;font-weight:600;">{escape(str(hdr_resolved_mode))}</div></div>
    </div>
</div>
"""
