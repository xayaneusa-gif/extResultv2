...
def do_verify_otp(otp: str):
    ...
    try:
        res_result_path = os.path.join(run_folder, "res_result.json")
        if os.path.exists(res_result_path):
            with open(res_result_path, "r", encoding="utf-8") as f:
                st.session_state.sem_debug_json = json.load(f)
            log(f"Loaded semester source JSON: {res_result_path}")
    except Exception as e:
        log(f"Semester source JSON load failed: {e}")
...
def do_generate_live(start_roll: int, end_roll: int, sem_value: dict, tone_mode: str, reappear_update_mode: str):
    ...
    if isinstance(sem_value, dict):
        log(
            "Using selected sem context: "
            f"semName={sem_value.get('semName')}, semNo={sem_value.get('semNo')}, "
            f"smId={sem_value.get('smId')}, esId={sem_value.get('esId')}, "
            f"examTypeKey={sem_value.get('examTypeKey')}, examScheduleName={sem_value.get('examScheduleName')}"
        )
...
with right:
    ...
    st.subheader("Sem Source JSON")
    sem_debug = st.session_state.get("sem_debug_json")
    if sem_debug:
        with st.expander("Show full res_result.json (all sem + exam schedules)"):
            st.json(sem_debug)
    else:
        st.info("Verify OTP to load full semester source JSON.")

    st.subheader("File Manager")
    fm_base_options = []
    current_run = st.session_state.get("run_folder")
    if current_run and os.path.exists(current_run):
        fm_base_options.append(("Current Run Folder", current_run))
    if os.path.exists(app_gui.RESP_BASE):
        fm_base_options.append(("All Runs Root", app_gui.RESP_BASE))

    if not fm_base_options:
        st.info("No accessible folder yet. Run OTP flow first.")
    else:
        fm_labels = [x[0] for x in fm_base_options]
        fm_choice = st.selectbox("Browse Scope", fm_labels, index=0, key="fm_scope")
        fm_base = next((p for lbl, p in fm_base_options if lbl == fm_choice), fm_base_options[0][1])
        st.caption(f"Base: {fm_base}")

        files = _walk_files(fm_base)
        if not files:
            st.info("No files found in selected scope.")
        else:
            filter_txt = st.text_input("Filter files", key="fm_filter", placeholder="e.g. res_result, .json, fetch_")
            if filter_txt.strip():
                q = filter_txt.strip().lower()
                files = [r for r in files if q in r[0].lower()]

            file_labels = [f"{rel} ({size} B)" for rel, _, size in files]
            picked_label = st.selectbox("Select File", file_labels, index=0, key="fm_pick")
            picked_idx = file_labels.index(picked_label)
            picked_rel, picked_abs, picked_size = files[picked_idx]

            st.code(picked_abs)
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Preview Text", use_container_width=True, key="fm_preview_btn"):
                    st.session_state["fm_preview_data"] = _safe_read_text(picked_abs)
                    st.session_state["fm_preview_path"] = picked_abs
            with c2:
                try:
                    st.download_button(
                        "Download File",
                        data=_read_binary(picked_abs),
                        file_name=os.path.basename(picked_abs),
                        use_container_width=True,
                        key="fm_download_btn",
                    )
                except Exception:
                    st.button("Download File", disabled=True, use_container_width=True, key="fm_download_btn_disabled")

            preview_path = st.session_state.get("fm_preview_path")
            preview_data = st.session_state.get("fm_preview_data")
            if preview_path == picked_abs and preview_data:
                with st.expander("File Preview"):
                    if picked_abs.lower().endswith(".json"):
                        try:
                            st.json(json.loads(preview_data))
                        except Exception:
